...
QHostAddress IPCtrl::getHostAddr()
{
    QString addrString(m_pLineEdit[0]->text() + "." + m_pLineEdit[1]->text() + "." + m_pLineEdit[2]->text() + "." + m_pLineEdit[3]->text());
    hostAddr.setAddress(addrString);
    return hostAddr;
}

void IPCtrl::setHostAddr(const QHostAddress &value)
{
    hostAddr = value;

    QStringList addr = hostAddr.toString().split('.');

    for (int i = 0; i != QTUTL_IP_SIZE; i++) {
      m_pLineEdit[i]->setText(addr.at(i));
    }
}
